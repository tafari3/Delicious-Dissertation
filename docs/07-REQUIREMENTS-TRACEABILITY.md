...
This document ensures the engineering build remains traceable to the academic proposal. It is not a replacement for the proposal; it is a map from proposal commitments to repository components and acceptance evidence.

## 2. Objective traceability

| Proposal objective / commitment | Engineering location | Required proof |
|---|---|---|
| Define a bounded OWASP-aligned API test catalogue | `docs/02-TEST-CATALOGUE.md`, `scanner/.../rules/` | Versioned rule catalogue and automated tests |
...
| Enforce allow-listing, rate limiting and non-destructive defaults | Safety preflight + controlled HTTP executor | Unit/integration tests demonstrating hard blocking/ceilings |
...
| Exclude DoS, credential stuffing, unrestricted fuzzing and exploitation beyond proof | charter, agent contract, HTTP/rule budgets | No such capability in code; hard stop tests for relevant limits |
