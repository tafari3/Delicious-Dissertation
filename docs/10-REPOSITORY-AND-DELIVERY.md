...
8. `docs/09-CONFIGURATION-DEFAULTS.md` — initial operational defaults;
9. `docs/07-IMPLEMENTATION-PLAN.md` — implementation order/phase gates;
10. `docs/08-REQUIREMENTS-TRACEABILITY.md` — proposal mapping;
11. `AGENTS.md` — agent execution procedure.
...
A phase is ready when preceding dependencies are merged/passing, relevant design docs are clear, acceptance tests can be described before coding, and no unresolved scope/safety conflict exists.

## 13. Definition of done

A phase is done when the stated implementation is complete, required automated tests pass, safety requirements hold, documentation is updated, CI passes at the exact PR head, acceptance proof is reproducible, and no placeholder is represented as working functionality.
